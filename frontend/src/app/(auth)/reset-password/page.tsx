...
export default function ResetPasswordPage() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await resetPassword.mutateAsync({ email, otp, new_password: newPassword });
      router.push('/login');
    } catch (err) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      setError((err as any)?.response?.data?.error?.message || 'Failed to reset password');
    }
  };
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-[420px] bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-[var(--radius-lg)] p-8 shadow-[var(--shadow-lg)]">
        ...
        <div className="mb-8">
          <h2 className="text-[var(--text-lg)] font-medium text-[var(--text-primary)]">New Password</h2>
          <p className="text-[var(--text-secondary)] text-[var(--text-sm)] mt-2">Enter the OTP sent to your email and a new password.</p>
        </div>
        ...
        <form onSubmit={handleReset} className="space-y-4">
          ...
          <div>
            <input 
              ...
              className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] focus:border-[var(--border-accent)] rounded-[var(--radius-md)] px-4 py-3 text-[var(--text-primary)] text-sm font-mono tracking-widest outline-none transition-colors"
              required 
            />
          </div>
